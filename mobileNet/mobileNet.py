...
class MobileNetV2(object):
    ...
    def blazeModel(self, x):
        with tf.variable_scope('BlazeNet'):
            ...
            output = self.BlazeBlock(output, 32, 1, 'BlazeBlock3', 2, False)
            output = self.BlazeBlock(output, 32, 1, 'BlazeBlock4', 1, False)
            output = self.BlazeBlock(output, 32, 1, 'BlazeBlock5', 1, False)
            output = self.BlazeBlock(output, 48, 1, 'BlazeBlock6', 2, True, 16)
            output = self.BlazeBlock(output, 48, 1, 'BlazeBlock7', 1, True, 24)
            output = self.BlazeBlock(output, 48, 1, 'BlazeBlock8', 1, True, 24)
            output = self.BlazeBlock(output, 64, 1, 'BlazeBlock9', 2, True, 32)
            output1 = self.BlazeBlock(output, 64, 1, 'BlazeBlock10', 1, True, 32)
